...
namespace TerrorbornMod.Items.Shadowcrawler
{
    public class BladeOfShade : ModItem
    {
        ...
        public override void OnHitNPC(Player player, NPC target, int damage, float knockBack, bool crit)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                Projectile.NewProjectile(player.GetProjectileSource_OnHit(target, ProjectileSourceID.None), target.Center, target.DirectionTo(mousePosition).RotatedByRandom(MathHelper.ToRadians(5)), ModContent.ProjectileType<NightmareBoilMelee>(), Item.damage / 2, knockBack, Owner: player.whoAmI);
            }
        }
    }
